  ...
  <div class="page-root">
    ...
    <header class="hud-header">
      <div class="header-inner">
        ...
        <!-- Status Chips -->
        <div class="flex items-center gap-3">
          <div class="status-chip" :class="apiUrlOk ? 'chip-ok' : 'chip-err'">
            ...
            <table class="risk-table" aria-live="polite">
              <thead>
                <tr>
                  ...
                  <th scope="col" class="th-cell cursor-pointer" @click="sortBy('confidence')">
                    信心分數 <span class="sort-icon">{{ sortIndicator('confidence') }}</span>
                  </th>
                  <th scope="col" class="th-cell">風險等級</th>
                  <th scope="col" class="th-cell">預測結論</th>
              ...
              <tbody>
                ...
                >
                  ...
                  <!-- ── 信心分數 + 動態色標 ──
                       ≥ 0.9 → 深紅   (extreme-risk)
                       0.7–0.9 → 橙色 (high-risk)
                       0.5–0.7 → 琥珀 (medium-risk)
                       < 0.5  → 冷綠  (low-risk)
                  ── -->
                  <td class="td-cell">
                    <div class="confidence-bar-wrap">
                      <div class="confidence-bar-track">
                        <div
                          ...
                          :class="confidenceBarCls(item.confidence)"
                          :style="{ width: (item.confidence * 100).toFixed(1) + '%' }"
                          role="meter"
                          :aria-valuenow="(item.confidence * 100).toFixed(1)"
                          aria-valuemin="0"
                          aria-valuemax="100"
                        ></div>
                      </div>
                      <span class="confidence-num" :class="confidenceTextCls(item.confidence)">
                  ...
                  <td class="td-cell text-center">
                    <span v-if="item.ai_prediction === 1" class="pred-flag pred-flagged">疑似洗錢</span>
                  ...
                  <!-- ── SAR Report 按鈕 ──
                       僅在 is_extreme_risk === true 且 sar_report 存在時顯示
                  ── -->
                  <td class="td-cell text-center">
                    <button
                      v-if="item.is_extreme_risk && item.sar_report"
                      class="btn-report"
                      :aria-label="`開啟 ${item.user_id} 的 SAR 報告`"
                    ...
                    <span v-else-if="item.is_extreme_risk" class="text-slate-500 text-xs">生成中…</span>
                ...
                <tr v-if="filteredPredictions.length === 0">
                  <td colspan="6" class="empty-row">無符合條件的記錄</td>
                </tr>
              </tbody>
            </table>
          </div>
...
async function startAnalysis() {
  ...
  try {
    ...
    // Step 2 — POST 純 CSV 字串
    // Content-Type: text/csv → Lambda 以 csv.DictReader 解析 Body
    // timeout: 30s（規格要求）；注意：若後端呼叫 Bedrock，實際需 60–120s
    //          如需容納長時間推論，請將 timeout 調整為 180_000
...
// ════════════════════════════════════════════════════════════════
// 動態風險色標（依 confidence 值）
// ≥ 0.90 → 深紅  (extreme)
// 0.70–0.90 → 橙  (high)
// 0.50–0.70 → 琥珀 (medium)
// < 0.50  → 冷綠  (low)
// ════════════════════════════════════════════════════════════════
function confidenceBarCls(conf) {
  if (conf >= 0.9)  return 'bar-extreme'
  if (conf >= 0.7)  return 'bar-high'
  if (conf >= 0.5)  return 'bar-medium'
  return 'bar-low'
}
function confidenceTextCls(conf) {
  if (conf >= 0.9)  return 'text-red-400'
  if (conf >= 0.7)  return 'text-orange-400'
...
/**
 * 風險等級判斷優先序：
 * 1. is_extreme_risk === true  → 極端風險（深紅）
 * 2. confidence ≥ 0.7         → 高風險（橙）
 * 3. confidence ≥ 0.5         → 中風險（琥珀）
 * 4. 其餘                      → 低風險（冷綠）
 */
function riskLevel(item) {
  if (item.is_extreme_risk)    return 'extreme'
  if (item.confidence >= 0.7)  return 'high'
  if (item.confidence >= 0.5)  return 'medium'
  return 'low'
}
function riskLabel(item) {
  return { extreme: '極端風險', high: '高風險', medium: '中風險', low: '低風險' }[riskLevel(item)]
}
function riskBadgeCls(item) {
  return { extreme: 'badge-extreme', high: 'badge-high', medium: 'badge-medium', low: 'badge-low' }[riskLevel(item)]
}
function rowCls(item) {
  return { extreme: 'row-extreme', high: 'row-high', medium: 'row-medium', low: '' }[riskLevel(item)]
}
...
const statCards = computed(() => {
  ...
  const extreme = list.filter(i => i.is_extreme_risk).length
  const high    = list.filter(i => !i.is_extreme_risk && i.confidence >= 0.7).length
  const medium  = list.filter(i => i.confidence >= 0.5 && i.confidence < 0.7).length
  const low     = list.filter(i => i.confidence < 0.5).length
  return [
    { label: '總筆數',   value: list.length, numCls: 'num-total',   tileCls: 'tile-total'   },
    { label: '極端風險', value: extreme,      numCls: 'num-extreme', tileCls: 'tile-extreme' },
    { label: '高風險',   value: high,         numCls: 'num-high',    tileCls: 'tile-high'    },
    { label: '中風險',   value: medium,       numCls: 'num-medium',  tileCls: 'tile-medium'  },
    { label: '低風險',   value: low,          numCls: 'num-low',     tileCls: 'tile-low'     },
  ]
})
